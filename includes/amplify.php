...
//        return $this;
    ...
    function http_call($functionName, $argumentsArray) {
            ...
            $this->_bot_detected();
        if (!$this->botDetect && $this->cookiesEnabled) {// checking if user is not a bot and cookies are enabled
            ...
            try {
                ...
                if (isset($argumentsArray) && is_array($argumentsArray) && count($argumentsArray) > 0) {
                    $argumentsArray['systemInfo'] = array('device' => $this->deviceDetect, 'os' => '', 'browser' => '', 'server_var' => $_SERVER['REMOTE_ADDR']);
...
//            setcookie('amplifysid', $response->amplifySession, time() + 604800, COOKIEPATH, COOKIE_DOMAIN);
//        }
        return $response;
    }
    ...
    private function _bot_detected() {
      //  $this->browser = $_browserInfo['browser'];
